...
function getDirectUrl(iCloudUrl) {
  // Convert iCloud share link to direct download
  const match = iCloudUrl.match(/iclouddrive\/([^#]+)#/);
  if (!match) throw new Error('Invalid iCloud URL');
  const id = match[1];
  return `https://www.icloud.com/iclouddrive/${id}/Logistic_report.xlsm`;
}
